...
find_best_model <- function(train_data, period = 52, seasonal = TRUE) {
  ...
  initial_model <- auto.arima(train_data, seasonal = seasonal)
...
xts_data <- xts(data_model$Count, order.by = as.Date(data_model$WeekDate))
...
all_data <- data.frame(
  date = all_dates,
  observed = all_obs,
  predicted = all_pred53,
  smoothed.obs = smoothed.obs,
  smoothed.pred = smoothed.pred,
  lower_PI95 = lwr95,
  upper_PI95 = upr95,
  lower_PI99 = lwr99,
  upper_PI99 = upr99,
  lower_PI95sm = lwr95.sm,
  upper_PI95sm = upr95.sm,
  lower_PI99sm = lwr99.sm,
  upper_PI99sm = upr99.sm
)
...
train_data <- all_data[all_data$date < forecast_start, ]

NCP.plot <- ggplot() +
  # Forecast 95% prediction interval ribbon
  ...
  geom_ribbon(data = train_data, aes(x = date, ymin = lower_PI95sm, ymax = upper_PI95sm, fill = "in-sample 95% prediction interval"), alpha = 0.6) +
  ...
  geom_ribbon(data = train_data, aes(x = date, ymin = lower_PI99sm, ymax = upper_PI99sm, fill = "in-sample 99% prediction interval"), alpha = 0.4) +
  ...
  geom_point(data = predicted_data, aes(x = date, y = predicted, color = "black"), shape = 2) +
  ...
  guides(
    color = guide_legend(
      ...
      override.aes = list(
        linetype = c(1, NA, NA, NA, NA, NA),  # Line type for Smoothed, None for Observed and Predicted
